...
public class GeoCoordinatesLatitudeIterator extends edu.uiowa.slis.FAST.TagLibSupport {
	static GeoCoordinatesLatitudeIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(GeoCoordinatesLatitudeIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://schema.org/latitude> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				latitude = sol.get("?s").toString();
				firstInstance = true;
